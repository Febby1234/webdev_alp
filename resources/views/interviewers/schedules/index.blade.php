        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    ...
                    {{-- Timeline Schedule --}}
                    <div class="space-y-4">
                        @forelse($schedules ?? [] as $time => $timeSchedules)
                                        ...
                                        <div class="bg-white rounded-lg p-4 mb-3 last:mb-0 shadow-sm hover:shadow-md transition">
                                            <div class="flex items-center justify-between">
                                                <div class="flex items-center flex-1">
                                                    ...
                                                    <div class="ml-4">
                                                        <h4 class="font-semibold text-gray-900">{{ $registration->personalDetail->full_name ?? '-' }}</h4>
                                                        <div class="flex items-center gap-3 mt-1">
                                                            <p class="text-sm text-gray-600">{{ $registration->registration_code }}</p>
                                                            <span class="text-sm text-gray-400">•</span>
                                                            <p class="text-sm text-gray-600">{{ $registration->major->name ?? '-' }}</p>
                                                        </div>
                                                    </div>
                                                </div>

                                                <div class="flex items-center gap-3">
                                                    @if($registration->examResults && $registration->examResults->count() > 0)
                                                    <div class="text-right mr-4">
                                                        <p class="text-sm text-gray-600">Nilai</p>
                                                        <p class="text-2xl font-bold text-green-600">{{ $registration->examResults->first()->score }}</p>
                                                    </div>
                                                    ...
                                                    <a href="{{ route('interviewer.participants.show', $registration->id) }}"
                                                       class="px-4 py-2 bg-purple-600 text-white text-sm rounded-lg hover:bg-purple-700 transition">
                                                        {{ $registration->examResults && $registration->examResults->count() > 0 ? 'Lihat' : 'Interview' }} →
                                                    </a>
                                                </div>
                                            </div>
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center">
                                    <div class="w-16 h-16 bg-purple-100 rounded-lg flex items-center justify-center flex-col">
                                        <p class="text-2xl font-bold text-purple-600">{{ $schedule->date->format('d') }}</p>
                                        <p class="text-xs text-purple-600">{{ $schedule->date->format('M') }}</p>
                                    </div>
                                    <div class="ml-4">
                                        <h4 class="font-semibold text-gray-900">{{ $schedule->type }}</h4>
                                        <p class="text-sm text-gray-600">{{ $schedule->date->format('l, d F Y') }}</p>
                                        <p class="text-sm text-gray-600">{{ $schedule->time }} WIB - {{ $schedule->location }}</p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    <p class="text-sm text-gray-600">Peserta</p>
                                    <p class="text-2xl font-bold text-gray-900">{{ $schedule->registrations_count ?? 0 }}</p>
                                </div>
                            </div>
                        </div>
                        @empty
