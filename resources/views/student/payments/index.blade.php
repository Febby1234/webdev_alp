    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            @if ($latest_payment && $latest_payment->status == 'verified')
                <div class="mb-6 bg-green-50 border border-green-200 rounded-lg p-4">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-green-800"><strong>Pembayaran Terverifikasi!</strong> Pembayaran sudah diverifikasi admin.</p>
                        </div>
                    </div>
                </div>
            @elseif($latest_payment && $latest_payment->status == 'pending')
                <div class="mb-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-yellow-800"><strong>Menunggu Verifikasi!</strong> Bukti pembayaran sedang diverifikasi.</p>
                        </div>
                    </div>
                </div>
            @endif
            ...
            <div class="bg-gradient-to-r from-blue-600 to-blue-700 rounded-lg shadow-lg p-8 mb-6 text-white">
                ...
                <span class="inline-flex items-center px-4 py-2 rounded-full text-sm font-medium {{ $latest_payment && $latest_payment->status == 'verified' ? 'bg-green-500' : ($latest_payment && $latest_payment->status == 'pending' ? 'bg-yellow-500' : 'bg-red-500') }}">
                    @if($latest_payment && $latest_payment->status == 'verified')
                        Lunas
                    @elseif($latest_payment && $latest_payment->status == 'pending')
                        Menunggu Verifikasi
            ...
            @if (!$latest_payment || $latest_payment->status == 'rejected')
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                    <div class="p-8 text-center">
                        ...
                        {{-- PERBAIKAN LINK DI SINI: Ke route create, bukan show --}}
                        <a href="{{ route('student.payments.create') }}" class="inline-flex items-center px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition shadow-sm">
                    ...
                    @forelse($payments ?? [] as $payment)
                        <div class="border-b last:border-0 py-4 flex items-center justify-between">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                @endphp
                                <span class="inline-flex px-3 py-1 rounded-full text-xs font-medium {{ $b['bg'] }} {{ $b['text'] }}">{{ $b['label'] }}</span>
